...
exports.getMatches = async (req, res) => {
  try {
    const userId = await utils.isIDGood(req.user._id)
    // const users_i_liked = user.users_i_liked ; 
    let MatchedUser = [] 
    return new Promise((resolve, reject) => {
        
 
        req.user.users_i_liked.forEach(element => {
          User.findById(element, (err, user) => {
            if (err) {
              reject(utils.buildErrObject(422, err.message))
            }
            else {
              if (user.users_i_liked.includes(userId)) {
                MatchedUser.push(user) 
                resolve(utils.buildSuccObject('user added to MatchedUser' ))  
              } 
              else {
                reject(utils.buildErrObject(422, err.message))
              }
        ...
        res.status(200).json({ 'MatchedUser': req.user.users_i_liked }) 
...
exports.getUserProfile = async (req, res) => {
  try {
    const userId = await utils.isIDGood(req.body._id)
    return new Promise((resolve, reject) => {
      User.findById(
        userId,
        (err, user) => {
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      User.findById(
        userId,
        (err, user) => {
          if (user.users_i_liked.includes(likedUserId)) {
            user.save((error) => {
              if (err) {
                reject(utils.buildErrObject(422, error.message))
              }
              res.status(200).json({ 'users_i_liked2': user.users_i_liked })
              resolve(utils.buildSuccObject('user already exists'))
            })
          }
          else {
            ...
            user.save((error) => {
              ...
              User.findById(
                likedUserId,
                (err, user) => {
                  if (user.users_who_liked_me.includes(userId)) {
                    user.save((error) => {
                      if (err) {
                        reject(utils.buildErrObject(422, error.message))
                      }
                    ...
                    user.save((error) => {
                      ...
                      res.status(200).json({ 'users_i_liked4': user.users_who_liked_me })
                      resolve(utils.buildSuccObject('user added'))
                    })
                  }
                })
